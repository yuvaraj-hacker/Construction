import React from "react";
import "../Home/Codes.css";
function Codes() {
  return (
    <>
      <br />
      <br />
      <br />
      <br />
      <br />
     
      <div className="second">
        <div
          className="urge"
          style={{
            width: "60px",
            height: "15px",
            backgroundColor: "transparent",
            border: "4px solid #f6b300",
            borderBottom: "none",
            marginTop: "25px",
            marginBottom: "20px",
            margin: "auto",
          }}
        ></div>
        <h1 className="white">OUR TEAMS</h1>
        <p style={{ fontFamily: "Inter,sans-serif" }}>
          HIGH PROFESSIONAL OF OUR ENGINEERS
        </p>
        <img
          style={{
            width: "60px",
            height: "3px",
            marginBottom: "100px",
            marginTop: "20px",
            margin: "auto",
          }}
          src="images/DASH.png"
        />
      </div>
      <div className="round">
        <div>
          <img className="to1" src="images/mohammed.jpg" />
          <p className="font1">Name</p>
        </div>
        <div>
          <img className="to1" src="images/amir.jpg" />
          <p className="font1">Name</p>
        </div>
        <div>
          <img className="to1" src="images/hossam.jpg" />
          <p className="font1">Name</p>
        </div>
        <div>
          <img className="to1" src="images/adul.jpg" />
          <p className="font1">Name</p>
        </div>
        <div>
          <img className="to1" src="images/arafa.jpg" />
          <p className="font1">Name</p>
        </div>
      </div>
      <div className="second">
        <div
          className="urge"
          style={{
            width: "60px",
            height: "15px",
            backgroundColor: "transparent",
            border: "4px solid #f6b300",
            borderBottom: "none",
            marginTop: "25px",
            marginBottom: "20px",
            margin: "auto",
          }}
        ></div>
        <h1 className="white">OUR PROCESS</h1>
        <p style={{ fontFamily: "Inter,sans-serif" }}></p>
        <img
          style={{
            width: "60px",
            height: "3px",
            marginBottom: "100px",
            marginTop: "20px",
            margin: "auto",
          }}
          src="images/DASH.png"
        />
      </div>
      <div className="back1">
        <div className="back2">
          <div className="with1">
            <div className="back3">
              <p  className="para2">01</p>
              <img src="images/icons8right.png" />
            </div>
            <p className="para3">SKETCHING</p>
            <p className="para4">
            Aenean commodo ligula eget dolor. <br/>
            Aeneane penatibus et mag penatibus<br/>
             et magnis nis dis parturient montes,<br/>
              nascetur ridiculus mus donec quam felis.<br/>
            </p>
          </div>
          <div className="with1">
            <div className="back3">
              <p className="para2">02</p>
              <img src="images/icons8right.png" />
            </div>
            <p className="para3">WORKING</p>
            <p className="para4">
            Aenean commodo ligula eget dolor.
            Aeneane penatibus et mag penatibus
             et magnis nis dis parturient montes,
              nascetur ridiculus mus donec quam felis.
            </p>
          </div>
          <div className="with1">
            <div className="back3">
              <p  className="para2">03</p>
              <img src="images/icons8right.png" />
            </div>
            <p className="para3">UTILIZATION</p>
            <p className="para4">
            Aenean commodo ligula eget dolor. <br/>
            Aeneane penatibus et mag penatibus<br/>
             et magnis nis dis parturient montes,<br/>
              nascetur ridiculus mus donec quam felis.<br/>
            </p>
          </div>
          <div className="with1">
            <div className="back3">
              <p  className="para2">04</p>
              <img src="images/icons8right.png" />
            </div>
            <p className="para3">DELIVERY</p>
            <p className="para4">
            Aenean commodo ligula eget dolor. <br/>
            Aeneane penatibus et mag penatibus<br/>
             et magnis nis dis parturient montes,<br/>
              nascetur ridiculus mus donec quam felis.<br/>
            </p>
          </div>
        </div>
      </div>
      <div className="second">
        <div
          className="urge"
          style={{
            width: "60px",
            height: "15px",
            backgroundColor: "transparent",
            border: "4px solid #f6b300",
            borderBottom: "none",
            marginTop: "25px",
            marginBottom: "20px",
            margin: "auto",
          }}
        ></div>
        <h1 className="white">OUR PROJECTS</h1>
        <p style={{ fontFamily: "Inter,sans-serif" }}></p>
        <img
          style={{
            width: "60px",
            height: "3px",
            marginBottom: "100px",
            marginTop: "20px",
            margin: "auto",
          }}
          src="images/DASH.png"
        />
      </div>
      <div className="rent1">
        <button className="butt0">
          <div className="find1">
            <p className="recent4">MADRIS PHOTOGRAPHER MARATHON</p>
            <p className="recen">
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Ut, totam
              saepe illo officia quos odit magnam accusamus, error iusto quas
              debitis voluptates, nemo reiciendis aut possimus impedit repellat
              rem inventore.
            </p>
            <button className="double">
              Read More
            </button>
          </div>
        </button>
        <button className="butt9">
          <div className="find1">
            <p className="recent4">CONSTRUCTION YOU CAN COUNT ON </p>
            <p className="recen">
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Ut, totam
              saepe illo officia quos odit magnam accusamus, error iusto quas
              debitis voluptates, nemo reiciendis aut possimus impedit repellat
              rem inventore.
            </p>
            <button className="double">
              Read More
            </button>
          </div>
        </button>
        <button className="butt8">
          <div className="find1">
            <p className="recent4">RECENT TRENDS IN STORY TELLING</p>
            <p className="recen">
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Ut, totam
              saepe illo officia quos odit magnam accusamus, error iusto quas
              debitis voluptates, nemo reiciendis aut possimus impedit repellat
              rem inventore.
            </p>
            <button className="double">
              Read More
            </button>
          </div>
        </button>
      </div>
      <div className="second">
        <div
          className="urge"
          style={{
            width: "60px",
            height: "15px",
            backgroundColor: "transparent",
            border: "4px solid #f6b300",
            borderBottom: "none",
            marginTop: "25px",
            marginBottom: "20px",
            margin: "auto",
          }}
        ></div>
        <h1 className="white">WHAT OUR CLIENTS SAY</h1>
        <p style={{ fontFamily: "Inter,sans-serif" }}></p>
        <img
          style={{
            width: "60px",
            height: "3px",
            marginBottom: "100px",
            marginTop: "20px",
            margin: "auto",
          }}
          src="images/DASH.png"
        />
      </div>
      <div className="child1">
        <img className="to1" src="images/image.20221001.jpeg" />
      </div>
      <div className="child2">
        <img className="rer1" src="images/icons8-double-quotes-50.png" />
      </div>
      <div className="der1">
        Lorem ipsum dolor sit amet consectetur adipisicing elit. <br />
        Labore ex similique saepe modi omnis. Praesentium numquam <br />
        totam exercitationem autem ipsa, pariatur, ut fugiat <br />
        mollitia vel nesciunt dolor veniam est alias! <br />
      </div>
      <div className="second">
        <div
          className="urge"
          style={{
            width: "60px",
            height: "15px",
            backgroundColor: "transparent",
            border: "4px solid #f6b300",
            borderBottom: "none",
            marginTop: "25px",
            marginBottom: "20px",
            margin: "auto",
          }}
        ></div>
        <h1 className="white">OUR SAFETY STANDARDS</h1>
        <p style={{ fontFamily: "Inter,sans-serif" }}></p>
        <img
          style={{
            width: "60px",
            height: "3px",
            marginBottom: "100px",
            marginTop: "20px",
            margin: "auto",
          }}
          src="images/DASH.png"
        />
      </div>
      <div>
      
        <ul className="fun1">
          <li>Personal Protective equipment</li>
          <li>Machinery Maintainence</li>
          <li>Personal protective equipment</li>
          <li>Keep the site clean</li>
          <li>Follow safety signs</li>
          <li>Machinery Maintenance</li>
          <li>Report</li>
          <li>Avoid unsafe areas</li>
          <li>Proper equipment</li>
          <li>Never tamper with equipment</li>
          <li>Ladders</li>
          <li>Provide the right equipment</li>
          <li>Training</li>
          <li>Communication</li>
          <li>Fall protection</li>
          <li>Jobsite Safety training</li>
          <li>Keep a tidy site</li>
          <li>Report defects and near misses</li>
          <li>Hazard Communication</li>
        </ul>
      </div>
     
    </>
  );
}

export default Codes;
